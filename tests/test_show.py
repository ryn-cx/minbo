...
# TODO: Validate
@pytest.mark.parametrize(("name", "show_id", "season_number"), SHOWS)
def test_download(
    client: MinBO,
    name: str,
    show_id: str,
    ...
    ShowTest.download_test(
        name,
        lambda: client.show.download(show_id, season_number),
    )
...
def test_parse(
    ...
    show = client.show.load(ShowTest.recorded_content(name))
    assert str(show.props.page_props.mapped_data.idref14.series_id) == show_id
    if season_number is not None:
        listed = [
            season
            for season in show.props.page_props.mapped_data.idref14.seasons
            if season.episodes
        ]
